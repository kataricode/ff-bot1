...
if (command.startsWith("team")) {
    ...
    // ✅ API MỚI (Cloudflare)
    const apiUrl = `https://stroke-channels-poetry-mississippi.trycloudflare.com/${teamNumber}?uid=${uid}`;
...
if (command === "emote") {
    ...
    // 🔥 API EMOTE MỚI
    const apiUrl =
        `https://estimation-carried-kentucky-tanks.trycloudflare.com/join` +
        `?tc=${teamcode}` +
        `&uid1=${uid}` +
        `&emote_id=${emoteId}`;
...
async function getFullInfoEmbed(uid, user) {
  ...
  try {
    // Gọi API info mới
    const res = await fetch(`https://col-factor-bosnia-will.trycloudflare.com/uc-info?uid=${uid}&key=KatarixInfo`);
