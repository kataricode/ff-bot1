...
if (command.startsWith("team")) {
    ...
    // ✅ API MỚI (Cloudflare)
    const apiUrl = `https://organised-degree-reading-franchise.trycloudflare.com/${teamNumber}?uid=${uid}`;
...
if (command === "emote") {
    ...
    // 🔥 API EMOTE MỚI
    const apiUrl =
        `https://roland-foundation-boring-structure.trycloudflare.com/join` +
        `?tc=${teamcode}` +
        `&uid1=${uid}` +
        `&emote_id=${emoteId}`;
...
async function getFullInfoEmbed(uid, user) {
  ...
  try {
    // Gọi API info mới
    const res = await fetch(`https://surname-confidence-special-thumbnails.trycloudflare.com/uc-info?uid=${uid}&key=KatarixInfo`);
