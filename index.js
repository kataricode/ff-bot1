...
if (command.startsWith("team")) {
    ...
    // ✅ API MỚI (Cloudflare)
    const apiUrl = `https://glucose-breaks-den-ages.trycloudflare.com/${teamNumber}?uid=${uid}`;
...
if (command === "emote") {
    ...
    // 🔥 API EMOTE MỚI
    const apiUrl =
        `https://plain-series-parliament-firewall.trycloudflare.com/join` +
        `?tc=${teamcode}` +
        `&uid1=${uid}` +
        `&emote_id=${emoteId}`;
...
async function getFullInfoEmbed(uid, user) {
  ...
  try {
    // Gọi API info mới
    const res = await fetch(`https://suit-music-aquarium-roster.trycloudflare.com/uc-info?uid=${uid}&key=KatarixInfo`);
