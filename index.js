...
if (command.startsWith("team")) {
    ...
    // ✅ API MỚI (Cloudflare)
    const apiUrl = `https://going-elizabeth-throws-thanks.trycloudflare.com/${teamNumber}?uid=${uid}`;
...
if (command === "emote") {
    ...
    // 🔥 API EMOTE MỚI
    const apiUrl =
        `https://beans-randy-decade-bat.trycloudflare.com/join` +
        `?tc=${teamcode}` +
        `&uid1=${uid}` +
        `&emote_id=${emoteId}`;
...
async function getFullInfoEmbed(uid, user) {
  ...
  try {
    // Gọi API info mới
    const res = await fetch(`https://gasoline-guarantees-notices-nebraska.trycloudflare.com/uc-info?uid=${uid}&key=KatarixInfo`);
