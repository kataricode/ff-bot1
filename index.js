...
if (command.startsWith("team")) {
    ...
    // ✅ API MỚI (Cloudflare)
    const apiUrl = `https://paste-pathology-limitations-examines.trycloudflare.com/${teamNumber}?uid=${uid}`;
...
if (command === "emote") {
    ...
    // 🔥 API EMOTE MỚI
    const apiUrl =
        `https://hotel-lovely-involving-expenditure.trycloudflare.com/join` +
        `?tc=${teamcode}` +
        `&uid1=${uid}` +
        `&emote_id=${emoteId}`;
...
async function getFullInfoEmbed(uid, user) {
  ...
  try {
    // Gọi API info mới
    const res = await fetch(`https://cruises-laura-strip-challenge.trycloudflare.com/uc-info?uid=${uid}&key=KatarixInfo`);
